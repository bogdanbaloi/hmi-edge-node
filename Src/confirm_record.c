...
uint32_t confirm_record_confirms(const uint8_t raw[CONFIRM_RECORD_BYTES],
    ...
    const uint32_t confirmed = le_read32(&raw[CONFIRM_RECORD_VERSION_AT]);
    return ((marker == CONFIRM_RECORD_MARKER) && (confirmed == version)) ? 1U
                                                                         : 0U;
}
